...
class OfferThemeService {
      ...
      fetch(this.URL, { method: "post", headers: headers, body: formData })
        ...
        .then((data) => {
          try {
            ...
            if (data.result == 1) {
              form.reset()
              this.offerThemeTable.querySelector(
                "tbody"
              ).innerHTML += `<tr data-id="${data.row.id}" class='table-themes__tr position-relative'><td>${data.row.name}</td></tr>`
    ...
    clickRow(row) {
      if (row.classList.contains("table-themes__tr--active")) {
        ...
        let activeRow = this.offerThemeTable.querySelector(
          ".table-themes__tr--active"
        )
        if (activeRow) {
        ...
        row.innerHTML +=
          "<button id='table-themes__btn-remove' title='Удалить'>🗑</button>"
