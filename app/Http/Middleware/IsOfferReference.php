...
class IsOfferReference
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if (array_key_exists('ref', $params)) {
            ...
            $time = OfferService::getDate();

            $refCode = $params['ref'];
            $subscription = OfferSubscription::where('refcode', $refCode)->first();
            if (is_null($subscription)) {
                Log::stack(['slack', $logChannel])->info("$time: переход по ссылке {$request->path()}?ref=$refCode завершился с ошибкой");
                return redirect('page404');
            } else {
                // зафиксировать факт перенаправления
                OfferClick::add($subscription->follower->id, $subscription->offer->id);
                Log::stack(['slack', $logChannel])->info("$time: переход по ссылке {$request->path()}?ref=$refCode успешен");
